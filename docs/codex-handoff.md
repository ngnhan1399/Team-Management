...
- **Đã bỏ hẳn CMS Browser Panel**: theo yêu cầu mới, `ArticlesPage.tsx` không còn mở khung preview bên phải nữa. Click tiêu đề bài viết giờ mở thẳng `review_link` hoặc `link` trong một named tab CMS (`cms_review`) để vẫn tái sử dụng tab cũ mà không làm rối UI chính.
...
- Luồng mở link duyệt bài hiện dùng `window.open(..., "cms_review")` trực tiếp từ danh sách bài; không còn panel CMS bên trong app nữa.
