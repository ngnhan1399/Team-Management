...
export default function ArticlesPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      const idleHandle = window.requestIdleCallback(() => runCheck(), { timeout: 1200 });
      return () => {
        controller.abort();
    ...
    const timer = globalThis.setTimeout(runCheck, 350);
    return () => {
      controller.abort();
  ...
  const openArticleModal = useCallback((article?: Article) => {
    if (article) {
      setFormData({
        ...article,
        status: article.status === "Approved" ? "Published" : article.status,
        wordCountRange: normalizeWordCountRangeValue(article.wordCountRange),
      });
    } else {
      setFormData({
        date: new Date().toISOString().split("T")[0],
        penName: canManageArticles ? "" : user?.collaborator?.penName,
    ...
    setShowModal(true);
    void ensureCollaboratorsLoaded();
  }, [canManageArticles, ensureCollaboratorsLoaded, user]);
  ...
  }, [ensureCollaboratorsLoaded, showFilters]);
    ...
    <>
      ...
      <header style={{ marginBottom: 32, display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          {canCreateArticles && (
            <button className="btn-ios-pill btn-ios-primary" onClick={() => openArticleModal()}>
            ...
            {canManageArticles && (
              <div className="form-group">
                ...
                <CustomSelect
                  ...
                  options={[{ value: "", label: collaboratorsLoading && collaborators.length === 0 ? "Đang tải CTV..." : "Tất cả CTV" }, ...collaborators.filter(c => c.role === "writer").map(c => ({ value: c.penName, label: c.penName }))]}
                  ...
                  <tr key={a.id} data-testid={`article-row-${a.id}`} style={{ borderBottom: "1px solid rgba(255, 255, 255, 0.02)", transition: "background 0.2s" }} className="hover:bg-white/[0.02]">
                    ...
                    <td style={{ padding: "12px 14px", textAlign: "center" }}>
                      <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "nowrap", whiteSpace: "nowrap" }}>
                        ...
                        {canEditArticle(a) && (
                          <button onClick={() => openArticleModal(a)} className="btn-ios-pill btn-ios-secondary" style={{ padding: "5px 9px", minWidth: 34, height: 34 }} title="Sửa">
      ...
      {showDeleteModal && canManageArticles && (
        <ArticleDeleteModal
          ...
          requestDeletePreview={requestDeletePreview}
